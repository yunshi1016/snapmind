...
struct AIService {
    ...
    不要输出 JSON、不要把整段回复包进代码块、不要任何额外说明。各项内容里可以自由使用任何标点（包括引号、换行）。
    ...
    ###OCR###
    （把截图里的文字转录成 Obsidian 兼容的 Markdown，尽量还原原排版结构：
    - 标题用 Markdown 标题，但**层级从 ### 起**（不要用 # 或 ##，以免破坏笔记结构）；
    - 列表用 - 或 1.；表格用 | 管道表格；代码/命令/配置用 ``` 围栏；强调用 **粗体**/*斜体*；链接用 [文字](网址)；
    - 段落之间空一行；保留文字原始语言，不翻译、不增删、不解说；
    - 截图中没有任何可读文字时，此项整段留空，不要写占位说明）
    ###TAGS###
