...
- **M11 来源链接** — 浏览器场景取当前页 URL：桌面端本地回环服务（`127.0.0.1:49219`）+ MV3 浏览器扩展推送活动标签页 URL；截图时若前台是浏览器则填入 `sourceUrl`。不内联 UI Automation。
...
> 注：曾试内联 UI Automation 直接读地址栏，但会触发 `0xc0000005` 访问违例且 Dart
> try/catch 无法捕获、拖垮整个进程，故改为「本地回环服务 + 浏览器扩展推送」方案（见 M11）。

## 明确不做（v1）

macOS/Android 客户端、视频录制、音频转写、账号系统、云同步、知识图谱、商业化计费、客户端 Docker 化。

（注：浏览器扩展原列入「不做」，但因来源链接 URL 的稳定方案就是它，已在 M11 落地——仅做 URL 推送这一件极小的事，不扩张职责。）
